...
# Lista para guardar planes de viaje
travel_plans = []
...
def main(page: ft.Page):
    ...
    def search_country(e):
        ...
        first_currency = list(currencies.values())[0].get("name", "N/A") if currencies else "N/A"

        languages = country.get("languages", {})
        languages_str = ", ".join(languages.values()) if languages else "N/A"

        # --- UI Output con 6 boxes ---
        info_section.current.content = ft.Column([
            ...
            # Primera fila: Capital, Region/Subregion, Population, Currency
            ft.Row([
                ft.Container(content=ft.Column([ft.Text("Capital", size=12, color=ft.Colors.GREY_600),
                                                ft.Text(capital, size=18, weight=ft.FontWeight.BOLD)], spacing=5),
                             padding=20, bgcolor=ft.Colors.BLUE_50, border_radius=12, expand=True),
                ft.Container(content=ft.Column([ft.Text("Region / Subregion", size=12, color=ft.Colors.GREY_600),
                                                ft.Text(f"{region} / {subregion}", size=18, weight=ft.FontWeight.BOLD)], spacing=5),
                             padding=20, bgcolor=ft.Colors.GREEN_50, border_radius=12, expand=True),
                ft.Container(content=ft.Column([ft.Text("Population", size=12, color=ft.Colors.GREY_600),
                                                ft.Text(f"{population:,}", size=18, weight=ft.FontWeight.BOLD)], spacing=5),
                             padding=20, bgcolor=ft.Colors.RED_50, border_radius=12, expand=True),
                ft.Container(content=ft.Column([ft.Text("Currency", size=12, color=ft.Colors.GREY_600),
                                                ft.Text(first_currency, size=18, weight=ft.FontWeight.BOLD)], spacing=5),
                             padding=20, bgcolor=ft.Colors.PURPLE_50, border_radius=12, expand=True),
            ], spacing=15),

            ft.Container(height=15),

            # Segunda fila: Languages y Time Zones
            ft.Row([
                ft.Container(content=ft.Column([ft.Text("Languages", size=12, color=ft.Colors.GREY_600),
                                                ft.Text(languages_str, size=18, weight=ft.FontWeight.BOLD)], spacing=5),
                             padding=20, bgcolor=ft.Colors.ORANGE_50, border_radius=12, expand=True),
                ft.Container(content=ft.Column([ft.Text("Time Zones", size=12, color=ft.Colors.GREY_600),
                                                ft.Text(timezones, size=18, weight=ft.FontWeight.BOLD)], spacing=5),
    ...
    def add_plan(e):
        ...
        travel_plans.append({"client": client, "country": country, "days": days, "notes": notes, "cost": cost})

        plans_table.current.rows.append(ft.DataRow(cells=[
    ...
    page.add(
        ft.Tabs(selected_index=0, tabs=[
            ft.Tab(text="Country Search", content=ft.Column([
                ft.Text("CARICOM Travel Agency", size=40, weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_700,
            ...
            ft.Tab(text="Trip Planning", content=ft.Column([
                ft.Text("Plan a Client Trip", size=28, weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_700),
                ...
                ft.Container(height=20),
                ft.DataTable(ref=plans_table, columns=[
